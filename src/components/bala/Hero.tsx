    ...
    >
      ...
      <div className="relative z-2 mx-auto max-w-[1280px] px-6 md:px-10 min-[1200px]:px-14">
        ...
        <div className="max-w-[640px]">
          <p className="inline-flex items-center gap-2.5 font-display text-xs tracking-[.14em] uppercase text-volt mb-5 animate-hero-in [animation-delay:.05s]">
            <span className="w-[7px] h-[7px] rounded-full bg-volt shadow-[0_0_0_4px_rgba(255,228,0,.18)]" />
            Ar tavo komandai pavyks pabėgti?
          </p>
          <h1 className="font-display uppercase text-white leading-[1.05] tracking-[-.015em] text-[clamp(32px,5.6vw,60px)] mb-5 animate-hero-in [animation-delay:.14s]">
            VR pabėgimo kambarys{" "}
            <span className="text-volt [text-shadow:0_0_38px_rgba(255,228,0,.45)]">Klaipėdoje</span>
          </h1>
          <p className="text-[clamp(18px,2.4vw,24px)] font-semibold leading-[1.3] text-white mb-6 animate-hero-in [animation-delay:.19s]">
            Tai ne žaidimas. <span className="text-volt">Tai nuotykis kitame pasaulyje.</span>
          </p>
          <div className="flex flex-col gap-2.5 max-w-[520px] mb-7 animate-hero-in [animation-delay:.24s]">
            <p className="text-[clamp(15px,1.4vw,16.5px)] leading-[1.55] text-smoke">
              Užsidedi VR akinius — ir po kelių sekundžių atsiduri drakonų pilyje, povandeninėje šventykloje arba paslaptingame dvare.
            </p>
            <p className="text-[clamp(15px,1.4vw,16.5px)] leading-[1.55] text-smoke">
              <span className="text-white font-semibold">Laikas bėga.</span> Tik jūs ir jūsų komanda.
            </p>
            <p className="text-[clamp(15px,1.4vw,16.5px)] leading-[1.55] text-smoke">Ar pavyks pabėgti ir išsigelbėti?</p>
          </div>
          <div className="flex flex-wrap gap-3.5 mb-7 animate-hero-in [animation-delay:.34s]">
            <a
              href="/rezervacija"
